                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="group">
                        ...
                        <select name="role" id="role_select" class="w-full px-5 py-4 bg-gray-50 border-gray-200 rounded-2xl focus:ring-2 focus:ring-emerald-600 outline-none transition font-bold text-gray-700 cursor-pointer">
                            ...
                            <option value="courier" {{ $currentRole == 'courier' ? 'selected' : '' }}>Courier / Kurir</option>
                        </select>
                        @error('role') <p class="text-red-500 text-xs mt-2 font-bold">{{ $message }}</p> @enderror
                    </div>

                    {{-- AGENCY (SHOW ONLY IF COURIER) --}}
                    <div class="group" id="agency_container" style="display: {{ $currentRole == 'courier' ? 'block' : 'none' }};">
                        ...
                        <select name="courier_agency_id" class="w-full px-5 py-4 bg-emerald-50/50 border-emerald-100 rounded-2xl focus:ring-2 focus:ring-emerald-600 outline-none transition font-bold text-emerald-900 cursor-pointer">
                            ...
                            @foreach($couriers as $agency)
                                <option value="{{ $agency->id }}" {{ old('courier_agency_id', $user->courier_agency_id) == $agency->id ? 'selected' : '' }}>
                ...
                <div class="mt-8 pt-8 border-t border-gray-200">
                    ...
                    <div class="bg-slate-50 rounded-2xl p-6 border border-slate-200">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            ...
                            <label class="permission-item flex items-start gap-3 p-4 bg-white rounded-xl border-2 border-transparent hover:border-emerald-300 transition-all cursor-pointer group shadow-sm">
                                <input type="checkbox"
                                    name="permissions[]"
                                    value="{{ $permission->name }}"
                                    data-permission="{{ $permission->name }}"
                                    {{ in_array($permission->name, $userPermissions) ? 'checked' : '' }}
                                    class="permission-checkbox mt-1 w-5 h-5 text-emerald-600 border-gray-300 rounded focus:ring-emerald-500 cursor-pointer transition-all">
                                <div class="flex-1">
            ...
            function updatePermissions(isInitial = false) {
                ...
                checkboxes.forEach(cb => {
                    ...
                    if (allowed.includes(name)) {
                        ...
                        item.classList.add('border-emerald-600', 'bg-emerald-50/20');
                        item.classList.remove('border-transparent');
                    } else {
                        if(!isInitial) cb.checked = false;
                        item.classList.remove('border-emerald-600', 'bg-emerald-50/20');
