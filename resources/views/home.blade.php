            ...
            <div class="flex gap-4 overflow-x-auto no-scrollbar pb-4">
                <template x-for="cat in categories" :key="cat">
                    <button @click="activeCategory = cat"
                        :class="activeCategory === cat ? 'bg-indigo-600 text-white shadow-lg' : 'bg-white text-gray-500 border-gray-100'"
                ...
                <template x-for="(item, index) in filteredItems" :key="item.id">
                    <div class="group bg-white rounded-[2.5rem] border border-gray-100 p-5 hover:shadow-2xl transition-all duration-500 flex flex-col h-full"
                        ...
                        <div class="relative aspect-square rounded-[2rem] overflow-hidden bg-gray-50 mb-6">
                            <img :src="item.gambar" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110">
                            <a :href="'/item-shop/' + item.id" class="absolute inset-0 bg-indigo-900/10 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center">
                                <div class="bg-white text-gray-900 px-6 py-2 rounded-xl font-black text-[10px] uppercase">Explore</div>
                            </a>
                        </div>
