            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                @auth
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button class="flex items-center gap-3 px-4 py-2 bg-gray-50/50 hover:bg-white hover:shadow-xl border border-transparent rounded-xl transition-all duration-300 group">
                                ...
                                <div class="w-10 h-10 rounded-full overflow-hidden border-2 border-white shadow-sm ring-1 ring-gray-100 transition-transform group-hover:scale-105">
                                    @if(Auth::user()->avatar)
                                        <img src="{{ asset(Auth::user()->avatar) }}" class="w-full h-full object-cover">
                                    @else
                                        <div class="w-full h-full bg-gradient-to-tr from-indigo-500 to-purple-500 flex items-center justify-center text-white font-black text-xs">
                        ...
                        <x-slot name="content">
                            <div class="p-2 space-y-1">
                                ...
                                <form method="POST" action="{{ route('logout') }}">
                                    @csrf
                                    <x-dropdown-link :href="route('logout')" class="rounded-xl font-bold text-xs uppercase tracking-widest text-red-600 hover:bg-red-50" onclick="event.preventDefault(); this.closest('form').submit();">
                                        {{ __('Log Out') }}
                                    </x-dropdown-link>
                                </form>
                            </div>
                ...
                @guest
                    <div class="flex items-center gap-4">
                        <a href="{{ route('login') }}" class="text-xs font-black uppercase text-gray-500 hover:text-indigo-600 transition-colors">Login</a>
                        <a href="{{ route('register') }}" class="text-xs font-black uppercase px-5 py-2.5 bg-indigo-600 text-white rounded-xl shadow-lg shadow-indigo-200 hover:bg-indigo-700 active:scale-95 transition-all">Register</a>
                    </div>
                @endguest
            </div>
        ...
        <div class="pt-4 pb-8 border-t border-gray-100 px-6">
            @auth
                <div class="flex items-center gap-4 bg-indigo-50/50 p-5 rounded-[2rem] mb-6 border border-indigo-100/50">
                    <div class="w-14 h-14 rounded-2xl overflow-hidden shadow-lg border-2 border-white ring-1 ring-indigo-200">
                        @if(Auth::user()->avatar)
                            <img src="{{ asset(Auth::user()->avatar) }}" class="w-full h-full object-cover">
                        @else
                            <div class="w-full h-full bg-indigo-600 flex items-center justify-center text-white font-black text-xl">
                                {{ strtoupper(substr(Auth::user()->name, 0, 1)) }}
                            </div>
                        @endif
                    </div>
                ...
                <div class="space-y-3">
                    <a href="{{ route('profile.edit') }}" class="w-full rounded-2xl font-black uppercase tracking-widest text-[11px] py-4 bg-white border border-gray-100 text-gray-700 shadow-sm flex justify-center items-center">
                        {{ __('Account Profile') }}
                    </a>
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="w-full rounded-2xl font-black uppercase tracking-widest text-[11px] py-4 bg-red-50 text-red-600 border border-red-100 flex justify-center items-center">
                            {{ __('Log Out Account') }}
                        </button>
                    </form>
                </div>
            ...
            @guest
                <div class="grid grid-cols-2 gap-4">
                    <a href="{{ route('login') }}" class="flex justify-center items-center py-4 rounded-2xl font-black uppercase tracking-widest text-[11px] bg-gray-50 text-gray-700 border border-gray-100">Login</a>
                    <a href="{{ route('register') }}" class="flex justify-center items-center py-4 rounded-2xl font-black uppercase tracking-widest text-[11px] bg-indigo-600 text-white shadow-lg shadow-indigo-100">Register</a>
                </div>
            @endguest
        </div>
    </div>
